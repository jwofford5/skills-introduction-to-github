...
- M.A.R. reference §4.2.4 — retained as intentional tactical emphasis (rejected by user)
-->
...
- All 10 highlight markers resolved
- §3.1 highlighted paragraph reformatted to clean prose
- "gear" → "equipment" (§4.2.1)
- "victim" → "victims" in Key Definitions (Rescue Teams entry)
- 14 layout annotations added
-->

<!-- POLISH CHANGES LOG
Changes made by doc-polish (Step 8):
  Acronyms defined on first use: 2 added (NFPA §2.2; PPE §7.4)
  Key Definitions additions: 1 (Rescue Group Supervisor / RGS)
  Version placeholder added: Yes (document header)
  Terminology unified: 0 additional (all consistent from prior steps)
  Cross-references: All verified accurate; 0 broken references found
  Remaining inspection issues: 22 total — 20 resolved, 1 retained (M.A.R./intentional), 1 flagged for SME (Level 3 vest spec)
  Protected sections (unchanged): None specified
-->

<!-- layout: COVER PAGE
  The document opens with a dedicated cover page (no page number) containing:
    ...
    Line 8: "Version 1.0 | Effective: [DATE]" — Times New Roman 10pt, centered, black
...
<!-- layout: DOCUMENT DEFAULTS
  ...
  Running header (pages 2+): document title left, page number right, Times New Roman 10pt
  No footer (page numbers in running header)
-->

<!-- layout: TABLE OF CONTENTS
  Replace plain-text TOC with Word auto-generated TOC.
  Include Heading 2 and Heading 3 only (not Heading 4).
  TOC on its own page; page break after before body.
-->
...
<!-- polish: version placeholder added -->
...
<!-- layout: SECTION BREAKS — Each major numbered section (##) begins on a new page. Use "Page Break Before" paragraph format on each Heading 2. -->
...
<!-- layout: CALLOUT BOX 1 — Reproduce in Word as a full-width shaded text box with dark red (#8B0000) header. See Layout Notes section for full specification. -->
...
<!-- layout: CALLOUT BOX 2 — Reproduce in Word as a full-width shaded text box with dark red (#8B0000) header. See Layout Notes section for full specification. -->
> **OPERATIONAL ZONE DEFINITIONS**
...
<!-- layout: Format radio example as left-indented block (0.5" left indent), italic, with thin dark red left border rule in Word. -->
...
<!-- polish: PPE defined on first use -->
...
<!-- layout: SME REVIEW FLAG — "Level 3 ballistic vest with plate" in §7.4 should be verified against current NIJ ballistic protection standards (Level III vs. Level IIIA) before final publication. -->
...
For large incidents with multiple RTFs, a Rescue Group Supervisor (RGS) is assigned to maintain accountability for all RTFs. Command deconflicts RTF operations with ongoing tactical operations to ensure rescue and tactical efforts complement rather than endanger each other.
...
<!-- layout: CHECKLIST FORMATTING — Use ☐ (Wingdings checkbox) with 0.25" hanging indent. Preserve exactly as shown; do not convert to Word's built-in checklist style. -->
...
<!-- layout: Format "PHASE 1 → PHASE 2:" and "PHASE 2 → PHASE 3:" labels as bold, dark red (#8B0000) text in Word. -->
...
<!-- layout: Each term bold, definition in regular weight, 6pt spacing after each entry. No bullets. -->
...
<!-- polish: RGS added to Key Definitions (was missing; flagged in inspection and Work Order) -->
**Rescue Group Supervisor (RGS):** The supervisor assigned to coordinate and maintain accountability of all Rescue Task Forces during a multi-RTF incident. The RGS manages RTF deployment, operations, and communications with Unified Command, and is typically a fire officer working in close coordination with a designated law enforcement counterpart.
...
<!-- layout: FIGURE 1 — Place on its own page after References. Full page width. Reproduce using Word SmartArt Hierarchy or borderless table. Box fill: light gray (#F2F2F2). Box borders and connectors: dark red (#8B0000), 1pt. Font: Times New Roman 10pt bold. Caption below in 10pt italic. -->

<!-- visual: Figure 1 alt-text
  Hierarchical org chart. Unified Command (Law Enforcement / Fire / EMS) at top.
  Two branches: Contact Group (Supervisor) left; Rescue Group (Supervisor) right.
  Contact Group splits into Contact Team 1 and Contact Team 2.
  Rescue Group splits into RTF 1 and RTF 2.
  Minimum viable URVI structure; expands based on incident conditions.
-->
...
|---|----------|------------------------|------------|--------|
| S1 | Structure | Section numbering inconsistency (1A vs 1.1) | Renamed to "1.1 Preamble" throughout — doc-structure | ✅ Resolved |
| S2 | Structure | Duplicate Operational Zone Definitions (callout + bullets) | Duplicate bullets removed; unique Warm Zone detail merged into callout box — doc-structure | ✅ Resolved |
| S3 | Structure | References section leaving blank last page | Section break and page layout annotations added; Figure 1 on its own page — doc-format | ✅ Resolved |
| L1 | Language | "leader's intent" phrasing (Low) | Retained — recognized military/government term, appropriate for audience | ✅ Accepted as-is |
| L2 | Language | M.A.R. vs MARCH inconsistency | Reviewed with user in FLAG-AND-CONFIRM; user determined M.A.R. is intentional tactical emphasis — retained | ✅ Accepted (intentional) |
| L3 | Language | Hyphens used as em-dashes in §7.2 | Corrected to em-dashes — doc-prose | ✅ Resolved |
| L4 | Language | "medics" inconsistent with "fire/EMS personnel" | Replaced in 2 instances (§7.3) — doc-prose | ✅ Resolved |
| F1 | Formatting | ~12 purple highlight markers unresolved | All 10 remaining highlights resolved — doc-format | ✅ Resolved |
| F2 | Formatting | Duplicate zone definitions visual clutter | Duplicate removed — doc-structure | ✅ Resolved |
| F3 | Formatting | Hyphen-dash inconsistency §7.2 | Corrected — doc-prose | ✅ Resolved |
| F4 | Formatting | Checkbox format (☐) confirmation for Word | Layout annotation added specifying Wingdings checkbox method — doc-format | ✅ Resolved |
| V1 | Visuals | Figure 1 caption adequacy | Caption improved with descriptive text; alt-text added — doc-visuals | ✅ Resolved |
| V2 | Visuals | Callout box reproduction in Word | Full design specification documented (dark red header, shaded body, border) — doc-visuals | ✅ Resolved |
| V3 | Visuals | Section ⊕ icon prefix from PDF | Not replicated — numbered section headings per government style are preferred and used throughout | ✅ Resolved (intentional omission) |
| Ly1 | Layout | Nearly blank last page | Page layout annotations added; content organized across pages — doc-format | ✅ Resolved |
| Ly2 | Layout | Running header replication in Word | Layout annotation added with exact running header specification — doc-format | ✅ Resolved |
| Ly3 | Layout | Page numbers in Word output | Included in running header layout annotation — doc-format | ✅ Resolved |
| P1 | Professionalism | TECC defined on first use | Already correctly handled in original — defined in §9.2 on first body use | ✅ No action needed |
| P2 | Professionalism | RGS not in Key Definitions | Added "Rescue Group Supervisor (RGS)" to Key Definitions — doc-polish | ✅ Resolved |
| P3 | Professionalism | M.A.R. not defined as standalone acronym | Retained intentionally (see L2 above) | ✅ Accepted (intentional) |
| P4 | Professionalism | THREAT Protocol not referenced in body | THREAT Protocol appears only in Key Definitions where it is defined; no body reference exists to fix | ✅ No action needed |
| P5 | Professionalism | No version number or effective date | "Version 1.0 | Effective: [DATE]" placeholder added to document header — doc-polish | ✅ Resolved |

### Terminology Unification Log

| Preferred Term | Replaced | Instances |
|----------------|----------|-----------|
| fire/EMS personnel | medics | 2 (§7.3) |
| equipment | gear | 1 (§4.2.1) |
| victims | victim | 1 (§12 Rescue Teams definition) |

No further terminology inconsistencies found. Document is consistent throughout.

### Acronym Status

| Acronym | First Defined | Status |
|---------|---------------|--------|
| URVI | Title / §1 intro | ✅ Defined on first use |
| EMS | §1 intro | ✅ Defined on first use |
| RTF / RTFs | §1.2 | ✅ Defined on first use |
| ICS | §1.5 | ✅ Defined on first use |
| NIMS | §1.5 | ✅ Defined on first use |
| NFPA | §2.2 (added by doc-polish) | ✅ Defined on first use |
| ICP | §4.1 Supervisor Responsibilities | ✅ Defined on first use |
| MARCH | §4.2 | ✅ Defined on first use |
| RGS | §4.2.5 | ✅ Defined on first use (body); now also in Key Definitions |
| TECC | §9.2 | ✅ Defined on first use |
| PPE | §7.4 (added by doc-polish) | ✅ Defined on first use |
| IC | Used as shorthand for Incident Commander after establishment | ✅ Acceptable — follows full term |
| CCP | Key Definitions §12 | ✅ Defined in Key Definitions; only abbreviated use is within same section |
| FD / LE | §11.4 checklist | ⚠️ Not formally defined — universally understood by target audience (public safety command staff); acceptable |

### Items Flagged for PM Audit

1. **Level 3 ballistic vest specification (§7.4):** The PPE specification "Level 3 ballistic vest with plate" requires subject matter expert verification against current NIJ protection level standards (Level III vs. Level IIIA) before final publication. Text retained as-is pending SME confirmation.

2. **FD / LE abbreviations (§11.4):** "FD" (Fire Department) and "LE" (Law Enforcement) appear in the RTF Deployment Checklist without formal definition. Acceptable for the document's target audience (public safety command staff) but noted for PM awareness.
