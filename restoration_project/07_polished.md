# Unified Response to Violent Incidents (URVI)

<!-- STRUCTURE CHANGES LOG
Changes made by doc-structure (Step 3):
1. Removed duplicate Operational Zone Definitions bullet points (kept callout box; merged unique Warm Zone sentence)
2. Renamed "1A. Preamble" → "1.1 Preamble" throughout (TOC and body)
3. Applied consistent markdown heading hierarchy (H1 title only; H2 major sections; H3 subsections; H4 sub-subsections)
-->

<!-- FLOW CHANGES LOG
Changes made by doc-flow (Step 4):
- Added 11 section lead-out transitions (one per major numbered section)
-->

<!-- PROSE CHANGES LOG
Changes made by doc-prose (Step 5) — Mode: FLAG-AND-CONFIRM:
- "medics" → "fire/EMS personnel" (2 instances, §7.3) — approved
- Em-dash correction §7.2 — approved
- "e.g.," abbreviation style §7.6 — approved
- M.A.R. reference §4.2.4 — retained as intentional tactical emphasis (rejected)
-->

<!-- VISUALS CHANGES LOG
Changes made by doc-visuals (Step 6):
- Figure 1: caption improved; alt-text added; Word SmartArt reproduction notes added
- Callout Box 1 & 2: Word design specification documented (dark red header, shaded body)
- 10 highlight flags cataloged with resolution recommendations
-->

<!-- FORMATTING CHANGES LOG
Changes made by doc-format (Step 7):
  Document type: Corporate / Government
  Header style: H2 = ALL CAPS numbered sections (confirmed consistent); H3 = Title Case; H4 = Title Case (confirmed consistent)
  List parallelism: Verified — all bullet lists use parallel grammatical structure throughout; no changes required
  Bold usage: Confirmed appropriate — bold reserved for defined terms in bullet lists, callout box headers, and key term labels; no changes required
  Tables: No body tables present; checklist format (☐ items) preserved as-is
  Highlight flags resolved: 10 of 10
    - §3.1 "coordinating": accepted, marker removed
    - §3.1 highlighted paragraph: reformatted — HIGHLIGHTED PARAGRAPH notation removed; text converted to clean prose paragraph
    - §4.2.1 "gear": changed to "equipment" (more formal; resolves reviewer flag)
    - §7.4 "Level 3 ballistic vest with plate": accepted, marker removed; SME review note added
    - §10.1 "across the county": accepted, marker removed
    - §10.1 "The first priority": accepted, marker removed
    - §10.4 "freelancing": accepted, marker removed
    - §12 "roles": accepted, marker removed
    - §12 "prior to establishing a fixed command post": accepted, marker removed
    - §12 "victim": changed to "victims" (plural; operationally accurate), marker removed
  Layout annotations added: 14
  Protected sections (unchanged): None specified
-->

<!-- POLISH CHANGES LOG
Changes made by doc-polish (Step 8):
  Acronyms defined on first use: 3 added
    - §2.2 "NFPA" → "National Fire Protection Association (NFPA)"
    - §4.1.4 "Incident Commander" → "Incident Commander (IC)" (establishes IC abbreviation for §§5, 10)
    - §7.4 "PPE" → "personal protective equipment (PPE)"
  Terminology unified: 1 term, 2 instances
    - §11.4 "FD and LE force protection officers" → "fire/EMS and law enforcement force protection officers" (consistency with body terminology)
  Cross-references verified: All TOC entries match body headings; all section lead-out references are accurate
  Key Definitions additions: 1
    - Added "Rescue Group Supervisor (RGS)" entry to §12 (per inspection issue Professionalism #2)
  Inspection issues addressed: 18 of 22 resolved by active skills; 2 flagged for PM audit; 2 deferred to doc-assemble (layout/page-economy decisions)
  Items flagged for PM:
    - Language #2 / Prof #3: M.A.R. vs MARCH unresolved (user rejected prose change as intentional tactical emphasis; conflicts with Work Order mandate to "resolve M.A.R. vs MARCH")
    - §1.5 cross-reference: "FIRESCOPE 701" in body vs. "FIRESCOPE California, ICS-701" in References — likely the same standard but cited inconsistently; subject matter expert verification recommended
  Protected sections (unchanged): None specified
-->

<!-- layout: COVER PAGE
  The document should open with a dedicated cover page (no page number) containing:
    Line 1: "UNIFIED RESPONSE TO VIOLENT INCIDENTS (URVI)" — Times New Roman 18pt, bold, centered, dark red (#8B0000)
    Line 2: blank
    Line 3: "Framework for Los Angeles County Operational Guidelines" — Times New Roman 13pt, italic, centered, black
    Line 4: blank
    Line 5: "Prepared by the URVI Guidelines Committee" — Times New Roman 11pt, centered, black
    Line 6: "Los Angeles County Public Safety Agencies" — Times New Roman 11pt, centered, black
    Line 7: blank (large gap — center the text block vertically on the page)
    Line 8: "Version 1.0 | Effective: [DATE]" — Times New Roman 10pt, centered, black [polish: version placeholder finalized — user to insert effective date prior to publication]
  Cover page uses Word section break (Next Page) before page 1 of body
-->

<!-- layout: DOCUMENT DEFAULTS
  Page size: Letter (8.5" × 11")
  Margins: 1 inch all sides
  Body font: Times New Roman 11pt
  Line spacing: 1.15 (body paragraphs); 1.0 (bullet lists and checklists)
  Paragraph spacing: 6pt after each paragraph; 12pt before each H3/H4 heading
  First page header/footer: blank (cover page)
  Running header (pages 2+): "Unified Response to Violent Incidents (URVI)" left-aligned, page number right-aligned — Times New Roman 10pt
  Running footer: blank
-->

<!-- layout: TABLE OF CONTENTS
  Replace the plain-text TOC with a Word auto-generated Table of Contents:
  Insert → Table of Contents → Automatic Table 1
  Apply to Heading 1 (document title), Heading 2 (major sections), Heading 3 (subsections)
  Do not include Heading 4 in the TOC
  TOC should appear on its own page, preceded by a "TABLE OF CONTENTS" heading (Heading 2 style)
  Insert page break after TOC before body begins
-->

---

*Framework for Los Angeles County Operational Guidelines*
Prepared by the URVI Guidelines Committee
...
1. EXECUTIVE SUMMARY — 3
   - 1.1 Preamble — 3
   - 1.2 Vision and Objectives — 3
   - 1.3 Target Audience — 3
   - 1.4 The Los Angeles County Operational Environment — 3
   - 1.5 Alignment with National Doctrine — 4
   - 1.6 Scope of Covered Incidents — 4
   - 1.7 Consequences of Breaking from Doctrine — 4

2. NATIONAL DOCTRINE FOUNDATION — 4
   - 2.1 Core NIMS Principles — 4
   - 2.2 NFPA 3000 as a Guiding Standard — 4

3. INCIDENT TYPES: DYNAMIC VS. STATIC SCENARIOS — 5
   - 3.1 Dynamic Incidents — 5
   - 3.2 Static or Contained Incidents — 5
   - 3.3 Transitioning Between Types — 5

4. THREE-PHASE RESPONSE MODEL — 5
   - 4.1 Phase 1 – Stop the Killing — 6
   - 4.1.1 Law Enforcement Priorities — 6
   - 4.1.2 Solo Officer Engagement — 6
   - 4.1.3 Contact Team Deployment — 6
   - 4.1.4 Maintaining Command During Tactical Entry — 6
   - 4.1.5 Supervisor Responsibilities — 7
   - 4.2 Phase 2 – Stop the Dying — 7
   - 4.2.1 Considerations for RTF Deployment — 7
   - 4.2.2 Operational Zone Definitions — 7
   - 4.2.3 RTF Functions — 7
   - 4.2.4 Tactical Emergency Medical Priorities — 8
   - 4.2.5 Coordinating Medical and Security Operations — 8
   - 4.3 Phase 3 – Recovery and Transition — 8
   - 4.3.1 Command Priorities — 8

5. COMMAND PROCEDURES — 9
   - 5.1 First Officer on Scene — 9
   - 5.1.1 Responsibilities — 9
   - 5.1.2 Initial Radio Communication — 9
   - 5.2 Mobile Command — 9
   - 5.3 Role of Second Supervisor — 9
   - 5.4 Transfer of Command — 9
   - 5.5 Transition from Mobile to Fixed Command Post — 10

6. UNIFIED COMMAND — 10
   - 6.1 Purpose and Importance — 10
   - 6.2 Triggers for Formation — 10
   - 6.3 Unified Command Responsibilities — 10
   - 6.4 Command Post Setup — 10
   - 6.5 Communication Protocols — 10
   - 6.6 Preventing Conflict in Multi-Agency Command — 11

7. RESCUE TASK FORCES (RTFS) — 11
   - 7.1 Definition and Composition — 11
   - 7.2 RTF Formation — 11
   - 7.3 Roles Within the RTF — 11
   - 7.4 Deployment Triggers and Priorities — 11
   - 7.5 Operational Cohesion — 12
   - 7.6 RTF-to-Command Communication — 12
   - 7.7 Withdrawal Criteria — 12
   - 7.8 Multiple RTF Oversight — 12

8. COMMUNICATIONS AND INTEROPERABILITY — 12
   - 8.1 Communications Principles — 12
   - 8.2 Radio Channel Architecture — 12
   - 8.3 Not All Agencies Can Communicate Directly — 12
   - 8.4 Information Flow During Operations — 13
   - 8.5 Subsequent Control Measures — 13

9. TRAINING AND PREPAREDNESS — 13
   - 9.1 Joint Training Imperative — 13
   - 9.2 Training for Phase 2 Operations — 13
   - 9.3 First-On-Scene Leadership Training — 13
   - 9.4 Pre-Incident Planning — 14
   - 9.5 Empowerment and Support — 14
   - 9.6 Sample Size-Up Components — 14

10. MUTUAL AID AND RESOURCE MANAGEMENT — 14
    - 10.1 Managing Incoming Resources — 14
    - 10.2 Mutual Aid Integration — 15
    - 10.3 Resource Tracking — 15
    - 10.4 Preventing Freelancing/Self-deployment and Overlap — 15

11. OPERATIONAL QUICK-REFERENCE — 15
    - 11.1 First Officer on Scene Checklist — 15
    - 11.2 Supervisor Arrival Checklist — 15
    - 11.3 Unified Command Formation Checklist — 16
    - 11.4 RTF Deployment Checklist — 16
    - 11.5 Phase Transition Decision Points — 16

12. KEY DEFINITIONS — 16
13. REFERENCES — 18

---

<!-- layout: SECTION BREAKS — Each major numbered section (##) should begin on a new page in the Word document. Insert a "Page Break Before" paragraph format on each Heading 2. Exception: the TOC page and cover page already have their own breaks. -->
...
### 1.4 The Los Angeles County Operational Environment
...
### 1.5 Alignment with National Doctrine
...
<!-- polish: cross-reference flag — "FIRESCOPE 701" in body vs. "FIRESCOPE California, ICS-701: Unified Response to Violence Incidents (2023)" in References (§13). Likely the same document, but cited inconsistently. Flagged for PM/SME verification before publication. -->

### 1.6 Scope of Covered Incidents

The framework covers violent incident types requiring unified response, including active assailant attacks (for example, shooters, mass stabbings, and vehicle rammings), mass casualty terrorist attacks (including bombings and coordinated attacks), civil unrest and riots involving violence, armed barricades, and hostage situations. This list is not all-inclusive, and there may be other violent incidents requiring force protection escorts by law enforcement. Purely accidental events or natural disasters are outside the scope unless compounded by deliberate violent acts.

### 1.7 Consequences of Breaking from Doctrine
...
The following section identifies the specific national standards—NIMS, ICS, and NFPA 3000—that form the doctrinal foundation upon which URVI is built.

---

## 2. NATIONAL DOCTRINE FOUNDATION

### 2.1 Core NIMS Principles
...
### 2.2 NFPA 3000 as a Guiding Standard

<!-- polish: acronym — "NFPA" defined on first use of full standard -->
National Fire Protection Association (NFPA) 3000 provides validated strategies for active shooter and hostile event response. Key tenets incorporated into URVI include Unified Command as a non-negotiable practice, integrated Rescue Task Forces entering warm zones under law enforcement protection, common operational language and compatible radio channels, and regular joint training exercises. By following NFPA 3000 guidance, URVI ensures Los Angeles County's practices meet nationally recognized benchmarks.

With this doctrinal foundation established, the framework next addresses how violent incidents are categorized—a distinction that directly shapes operational tempo, RTF deployment timing, and tactical approach.

---

## 3. INCIDENT TYPES: DYNAMIC VS. STATIC SCENARIOS
...
### 3.2 Static or Contained Incidents
...
### 3.3 Transitioning Between Types
...
The three-phase response model described in the following section provides the operational structure that applies across both incident types, enabling smooth transitions regardless of how a scenario begins or evolves.

---

## 4. THREE-PHASE RESPONSE MODEL

The URVI guidelines organize the response to violent incidents into three distinct phases.

<!-- layout: CALLOUT BOX 1 — Reproduce in Word as a full-width shaded text box with dark red (#8B0000) header. See CALLOUT BOX DESIGN SPECIFICATION in 05_visuals.md. Position immediately after the introductory paragraph above. -->
...
The first officer on scene must act without delay if lives are at risk, even without backup. Solo engagement is warranted when there is active gunfire or assault in progress, an immediate threat to multiple victims, and no other units are immediately available. Solo officers establish Mobile Command and move toward the threat, broadcasting their location and actions to incoming units. Solo engagement is a stopgap until reinforcements arrive.
...
<!-- polish: acronym — "Incident Commander (IC)" defined on first use; abbreviation used in §§5 and 10 -->
Tactical urgency must not eclipse command and control. Once a field supervisor arrives, that supervisor should assume command rather than joining the entry team. The supervisor establishes a command post, organizes additional arriving units, manages communications, and coordinates with incoming agencies. Not every law enforcement officer should self-deploy into the building; the Incident Commander (IC) exercises discipline over responding units, directing them to perimeter, staging, or specific tactical assignments.
...
- **RTF Team Equipped:** Personnel identified, wearing ballistic protective equipment, carrying hemorrhage control and rapid patient movement equipment.
...
<!-- layout: CALLOUT BOX 2 — Reproduce in Word as a full-width shaded text box with dark red (#8B0000) header. See CALLOUT BOX DESIGN SPECIFICATION in 05_visuals.md. Position immediately after the section heading above. -->
> **OPERATIONAL ZONE DEFINITIONS**
...
<!-- polish: "M.A.R." retained per user decision in doc-prose Step 5 (intentional tactical emphasis). Note: Work Order step 8 mandate called for resolving M.A.R. vs MARCH; user's explicit prose-step decision overrides. Flagged for PM audit. -->
...
### 4.3 Phase 3 – Recovery and Transition
...
The effective execution of all three phases depends on sound command procedures—beginning the moment the first responder arrives on scene, as described in the following section.

---

## 5. COMMAND PROCEDURES

### 5.1 First Officer on Scene
...
<!-- layout: The radio communication example below should be formatted in Word as a left-indented block (0.5" left indent), in italics, with a thin left border rule in dark red (#8B0000) — visually distinguishing it as an example/quotation. -->
...
### 5.3 Role of Second Supervisor

When a second supervisor arrives, they should typically assume the Incident Commander role, freeing the first supervisor or officer to focus on tactical operations. The second supervisor's arrival creates a critical division of labor: one leader manages the tactical fight, the other coordinates the overall incident.

### 5.4 Transfer of Command
...
### 5.5 Transition from Mobile to Fixed Command Post
...
The transition to a fixed command post marks the natural point at which multi-agency Unified Command is fully established—the structure described in the following section.
...
### 6.6 Preventing Conflict in Multi-Agency Command
...
Among Unified Command's most critical operational responsibilities during Phase 2 is the deployment and management of Rescue Task Forces—the integrated police-fire/EMS teams described in the following section.

---

## 7. RESCUE TASK FORCES (RTFs)
...
### 7.3 Roles Within the RTF

Law enforcement officers serve as the protective overwatch. They lead movement, secure corridors and doorways, maintain 360-degree force protection around fire/EMS personnel, and control the pace. Officers do not separate from the RTF or pursue independent actions. Their sole mission is RTF team safety.

Fire/EMS personnel focus on medical operations: rapid assessment, life-saving interventions (following the MARCH algorithm, e.g., tourniquets, wound packing, chest seals, airway management), triage decisions, and patient evacuation. They operate under the tactical parameters set by law enforcement; if officers say a location is not secure, fire/EMS personnel adapt accordingly.

### 7.4 Deployment Triggers and Priorities

<!-- polish: acronym — "personal protective equipment (PPE)" defined on first use -->
RTF deployment requires a designated warm zone, sufficient force protection, and appropriately equipped RTF teams (URVI personal protective equipment (PPE): ballistic helmet, Level 3 ballistic vest with plate, and no turnout gear). If resources are limited, command deploys to where immediate intervention will save the most lives and assigns subsequent teams as they become available.

<!-- layout: SME REVIEW FLAG — "Level 3 ballistic vest with plate" in §7.4 should be verified against current NIJ ballistic protection standards (Level III vs. Level IIIA) before final publication. This is a technical specification that requires subject matter expert confirmation. -->

### 7.5 Operational Cohesion

RTF members move together and stay together at all times. The team does not split up. Law enforcement and fire/EMS maintain visual contact throughout operations. Every movement is communicated and acknowledged within the team. If one member pauses (treating a patient or reloading), the entire team pauses. Anyone on the team should speak up to improve communications or voice safety concerns—this is encouraged and expected.

### 7.6 RTF-to-Command Communication
...
The effectiveness of RTF operations—and of all incident coordination—depends on reliable, disciplined communications across all agencies, as addressed in the following section.

---

## 8. COMMUNICATIONS AND INTEROPERABILITY

### 8.1 Communications Principles
...
### 8.2 Radio Channel Architecture
...
### 8.3 Not All Agencies Can Communicate Directly
...
### 8.4 Information Flow During Operations
...
### 8.5 Subsequent Control Measures
...
The communications discipline described in this section does not emerge spontaneously in the field—it is built through the joint training and preparedness activities detailed in the following section.

---

## 9. TRAINING AND PREPAREDNESS

### 9.1 Joint Training Imperative
...
### 9.2 Training for Phase 2 Operations
...
### 9.3 First-On-Scene Leadership Training
...
Even the best-trained agencies must manage the practical challenges of multi-jurisdictional resource coordination when a major incident draws units from across the county—the subject of the following section.

---

## 10. MUTUAL AID AND RESOURCE MANAGEMENT
...
### 10.4 Preventing Freelancing/Self-deployment and Overlap
...
The quick-reference checklists in the following section consolidate the key decision points and responsibilities from Sections 4 through 10 into an actionable format for use in the field.

---

## 11. OPERATIONAL QUICK-REFERENCE

<!-- layout: CHECKLIST FORMATTING — Section 11 checklists use ☐ checkbox items. In Word, format each ☐ item using the Wingdings "☐" character (or Insert → Symbol → Wingdings, char 111) in a consistent list style with 0.25" hanging indent. Do NOT convert to Word's built-in checklist style — it changes the visual appearance. Preserve exactly as shown. -->

### 11.1 First Officer on Scene Checklist
...
### 11.2 Supervisor Arrival Checklist
...
### 11.3 Unified Command Formation Checklist
...
### 11.4 RTF Deployment Checklist

<!-- polish: terminology — "FD and LE" → "fire/EMS and law enforcement" (consistency with body terminology elsewhere) -->
- ☐ Warm zone confirmed by law enforcement.
- ☐ RTF teams (fire/EMS and law enforcement force protection officers) briefed and assigned.
...
### 11.5 Phase Transition Decision Points

<!-- layout: PHASE TRANSITION HEADERS — "PHASE 1 → PHASE 2:" and "PHASE 2 → PHASE 3:" should be formatted in Word as bold, dark red (#8B0000) text to visually distinguish them as decision-point labels. -->
...
For the full definitions of key terms used throughout this framework, refer to the Key Definitions section below.

---

## 12. KEY DEFINITIONS

<!-- layout: KEY DEFINITIONS — In Word, format each definition entry with the term in bold (Times New Roman 11pt bold) followed by the definition in regular weight. Use 6pt spacing after each entry. Do not use bullet points. -->

**Active Assailant:** An individual or individuals actively engaged in killing or attempting to kill people in a populated area, typically using firearms but potentially using any weapon.

**Casualty Collection Point (CCP):** A designated safe area (typically in the cold zone) where victims are brought for further triage, treatment, and preparation for ambulance transport.

**Cold Zone:** The safe area surrounding an incident, free of active threat, where command posts, staging, and medical treatment areas are established.

**Contact Team:** A small group of law enforcement officers (typically 2–4) who move together to locate and neutralize an active threat.

**Force Protection:** Law enforcement officers assigned to provide armed security for fire/EMS personnel operating in warm zones.
...
**NFPA 3000:** The National Fire Protection Association standard for active shooter/hostile event response programs, establishing best practices for unified response.

**NIMS:** The National Incident Management System, providing principles for multi-agency coordination including common terminology, unified command, and resource management.

**Rescue Group Supervisor (RGS):** <!-- polish: NEW ENTRY — added per inspection issue Professionalism #2 -->The leader assigned to coordinate the deployment, operations, and accountability of all Rescue Task Forces during an incident. Typically a fire officer, with a designated law enforcement counterpart for close coordination; both supervisors are ideally co-located, with one focusing on victim care and the other on security.

**Rescue Task Force (RTF):** A unified team of fire/EMS personnel and law enforcement officers formed to treat and evacuate victims in warm zones under armed protection.

**Rescue Team(s):** Law enforcement resources whose mission it is to enter the Hot Zone to rescue victims. They are configured to search for and extract the injured from the Hot Zone to the Warm Zone, a CCP, or the Treatment Area/Cold Zone.

**TECC (Tactical Emergency Casualty Care):** Civilian adaptation of military combat casualty care principles, guiding medical treatment in high-threat environments with phases for direct threat, indirect threat, and evacuation care.

**THREAT Protocol:** Response model from the Hartford Consensus: Threat suppression, Hemorrhage control, Rapid Extrication, Assessment, and Transport.

**Unified Command:** A leadership structure where commanders from multiple agencies jointly manage an incident, sharing authority, setting common objectives, and developing a single action plan.

**Warm Zone:** An area cleared by law enforcement but not fully secured, where RTFs operate under armed escort. Personnel wear ballistic protective equipment and remain prepared for rapid withdrawal.
...
- C3 Pathways, Active Shooter Incident Management Checklist Help Guide.
- California Governor's Office of Emergency Services, Law Enforcement Guide for Emergency Operations (2019).
- Federal Emergency Management Agency, National Incident Management System, 3rd ed. (2017).
- Federal Emergency Management Agency, ICS-100: Introduction to the Incident Command System (2018).
- FIRESCOPE California, ICS-701: Unified Response to Violence Incidents (2023).
- Hartford Consensus, THREAT Protocol for Active Shooter/Mass Violence Response.
- International Association of Chiefs of Police, Active Shooter: Model Policy (2014).
- National Fire Protection Association, NFPA 3000: Standard for an Active Shooter/Hostile Event Response (ASHER) Program (2018).
- National Fire Protection Association, NFPA 1561: Standard on Emergency Services Incident Management System and Command Safety (2019).
- National Tactical Officers Association, Active Shooter—ASIM Checklist (2015).
- U.S. Fire Administration, Fire/EMS Operational Considerations and Guide for Active Shooter and Mass Casualty Incidents (2013).

---

## FIGURE 1: EXAMPLE BASIC URVI ORGANIZATION

<!-- layout: FIGURE 1 PLACEMENT — Place Figure 1 on its own page after the References section. Full page width. Preferred Word method: SmartArt → Hierarchy → Organization Chart. Box fill: light gray (#F2F2F2). Box border and connector lines: dark red (#8B0000), 1pt. Font: Times New Roman 10pt bold. Caption below chart in Times New Roman 10pt italic. -->

<!-- visual: Figure 1 alt-text
  Hierarchical org chart with Unified Command (Law Enforcement / Fire / EMS) at top.
  Two branches below: Contact Group (Supervisor) on the left, Rescue Group (Supervisor) on the right.
  Contact Group splits into Contact Team 1 and Contact Team 2.
  Rescue Group splits into RTF 1 and RTF 2.
  Represents minimum viable URVI structure; expands based on incident conditions.
-->
...
```
                   ...
                   (Law Enforcement / Fire / EMS)
                              |
              ┌───────────────┴───────────────┐
        Contact Group                    Rescue Group
         (Supervisor)                    (Supervisor)
              |                               |
       ┌──────┴──────┐                 ┌──────┴──────┐
  Contact Team 1  Contact Team 2     RTF 1         RTF 2
```

*Figure 1: Example basic organizational structure for a URVI incident response, showing the two-branch command hierarchy under Unified Command. Law enforcement Contact Groups manage threat neutralization; Rescue Groups coordinate RTF medical and extraction operations. Structure expands or contracts based on incident conditions, available resources, and Unified Command direction.*

---

## Layout Notes for Final Assembly

The following instructions summarize all layout decisions for the `/doc-assemble` skill to apply when generating `FINAL_DOCUMENT.docx`.

**Cover Page.** The document opens with a dedicated cover page (no page number). Title: "UNIFIED RESPONSE TO VIOLENT INCIDENTS (URVI)" in Times New Roman 18pt bold, dark red (#8B0000), centered. Subtitle: "Framework for Los Angeles County Operational Guidelines" in 13pt italic. Agency lines below in 11pt. Version/date placeholder at bottom center: "Version 1.0 | Effective: [DATE]" in 10pt — polish has finalized this as the version line; user inserts the effective date prior to publication. Use a Word section break (Next Page) after the cover before body page 1.

**Document Defaults.** Letter paper, 1-inch margins all sides. Body: Times New Roman 11pt, 1.15 line spacing, 6pt paragraph spacing after. Running header (pages 2+): document title left, page number right, Times New Roman 10pt. No footer.

**Table of Contents.** Replace the plain-text TOC with a Word auto-generated TOC (Insert → Table of Contents). Include Heading 2 and Heading 3 levels only (not Heading 4). TOC on its own page with a page break after.

**Section Breaks.** Each Heading 2 (major numbered section) uses "Page Break Before" paragraph formatting so every major section starts at the top of a new page.

**Heading Styles.** Heading 1 (document title): Times New Roman 18pt, bold, dark red (#8B0000). Heading 2 (numbered sections): Times New Roman 14pt, bold, ALL CAPS, black, with a 1pt dark red horizontal rule below. Heading 3 (subsections): Times New Roman 12pt, bold, Title Case, black. Heading 4 (sub-subsections): Times New Roman 11pt, bold, italic, black. All headings: `keep_with_next = True`.

**Callout Boxes (2).** Both the "Three-Phase Response Model" box (Section 4) and the "Operational Zone Definitions" box (Section 4.2.2) are reproduced as Word text boxes: full paragraph width, dark red (#8B0000) header row with white bold ALL CAPS text, light gray (#F2F2F2) body, 1.5pt dark red outer border. See full specification in `05_visuals.md`.

**Checklist Items.** Section 11 checklists use ☐ (Wingdings checkbox character). Format with 0.25" hanging indent. Do not convert to Word's built-in checklist style.

**Phase Transition Headers.** "PHASE 1 → PHASE 2:" and "PHASE 2 → PHASE 3:" formatted bold, dark red (#8B0000).

**Radio Communication Example (§5.1.2).** Format as a left-indented block (0.5" left indent), italic, with a thin dark red left border rule — visually distinguishing it as an example quotation.

**Key Definitions (Section 12).** Each term in bold, definition in regular weight, 6pt spacing after each entry. No bullets.

**Figure 1.** Placed on its own page after Section 13 (References). Reproduce using Word SmartArt Hierarchy or borderless table. Box fill: light gray (#F2F2F2). Box borders and connector lines: dark red (#8B0000), 1pt. Caption below in 10pt italic.

**Page Economy (References).** Inspection Layout #1 / Structure #3 flagged the References section as ending with one entry alone on a blank-ish final page. The Word auto-flow with the heading-styles above will reflow References onto the available section pages. If a blank page still appears after assembly, reduce paragraph spacing on the References list from 6pt to 3pt before adding Figure 1 on its own page.

**SME Review Flag.** The PPE specification "Level 3 ballistic vest with plate" in Section 7.4 requires subject matter expert verification against current NIJ ballistic protection standards before final publication.
...
|---|----------|------------------------|------------|--------|
| 1 | Structure | TOC mixes "1A" and "1.1" subsection notation | Renumbered "1A. Preamble" → "1.1 Preamble" (doc-structure) | ✅ Resolved |
| 2 | Structure | Duplicate Operational Zone Definitions (callout + bullets) | Bullets removed; callout box retained (doc-structure) | ✅ Resolved |
| 3 | Structure | Final reference alone on page 24 (blank page) | Layout Note added (References page economy); auto-flow expected to resolve, with fallback (doc-polish + doc-assemble) | ⏭ Deferred to assemble |
| 4 | Language | Preamble "leader's intent" awkward phrasing | Retained — used correctly as a military doctrinal term; intentional choice | ✅ Resolved (no change) |
| 5 | Language | M.A.R. vs MARCH inconsistency (§4.2.4) | doc-prose presented in FLAG-AND-CONFIRM; user rejected change as intentional tactical emphasis | ⚠️ Flagged for PM |
| 6 | Language | Hyphen-dash inconsistency (§7.2) | Em-dashes applied (doc-prose) | ✅ Resolved |
| 7 | Language | "medics" inconsistent with "fire/EMS personnel" | Two instances changed to "fire/EMS personnel" (doc-prose) | ✅ Resolved |
| 8 | Formatting | ~12 purple highlight flags throughout | 10 resolved in doc-format + 2 reconciled in prose log = 12 of 12 | ✅ Resolved |
| 9 | Formatting | Duplicate zone definitions visual clutter | Resolved structurally (see #2) | ✅ Resolved |
| 10 | Formatting | Hyphen-dash inconsistency (§7.2) | Resolved (see #6) | ✅ Resolved |
| 11 | Formatting | Checkbox (☐) Word translation | Wingdings spec documented in Layout Notes (doc-format) | ✅ Resolved |
| 12 | Visuals | Figure 1 caption / quality | Caption improved; alt-text added; SmartArt reproduction notes added (doc-visuals) | ✅ Resolved |
| 13 | Visuals | Callout boxes faithful reproduction | Word design specification documented (doc-visuals) | ✅ Resolved |
| 14 | Visuals | ⊕ section icon prefix | Implicitly removed during markdown conversion; not carried into Word output (doc-format implicit) | ✅ Resolved (removed) |
| 15 | Layout | Page 24 nearly blank | See #3 | ⏭ Deferred to assemble |
| 16 | Layout | Running header replication | Specified in Layout Notes (doc-format) | ✅ Resolved |
| 17 | Layout | Page number replication | Specified in Layout Notes (doc-format) | ✅ Resolved |
| 18 | Professionalism | TECC defined on first use | Confirmed at §9.2 — no change needed | ✅ Resolved (confirmed) |
| 19 | Professionalism | RGS not in Key Definitions | Added new entry to §12 (doc-polish) | ✅ Resolved |
| 20 | Professionalism | M.A.R. partial acronym vs MARCH | See #5 | ⚠️ Flagged for PM |
| 21 | Professionalism | THREAT Protocol first-use definition | Not used in document body; definition retained in §12 for reference. No first-use issue exists. | ✅ Resolved (confirmed) |
| 22 | Professionalism | No version number / effective date | "Version 1.0 | Effective: [DATE]" placed on cover (doc-format layout); polish finalized as the version line | ✅ Resolved |

### Terminology Unification Log

| Term Unified | Replaced | Instances | Step |
|---|---|---|---|
| fire/EMS personnel | medics | 2 | doc-prose |
| equipment | gear | 1 | doc-format |
| fire/EMS and law enforcement | FD and LE | 1 (§11.4) | doc-polish |
| victims | victim | 1 (§12 Rescue Teams) | doc-format |

### Acronym Status

| Acronym | Defined on First Use | Location | Status |
|---|---|---|---|
| URVI | Yes — title page and §1 | Title / §1 | ✅ |
| EMS | Yes — "emergency medical services (EMS)" | §1 | ✅ |
| RTF / RTFs | Yes — "Rescue Task Forces (RTFs)" | §1.2 | ✅ |
| NIMS | Yes — "National Incident Management System (NIMS)" | §1.5 | ✅ |
| ICS | Yes — "Incident Command System (ICS)" | §1.5 | ✅ |
| NFPA | Yes — "National Fire Protection Association (NFPA) 3000" | §2.2 | ✅ (added by doc-polish) |
| MARCH | Yes — full expansion at first use | §4.2 | ✅ |
| ICP | Yes — "Incident Command Post (ICP)" | §4.1.5 | ✅ |
| IC | Yes — "Incident Commander (IC)" | §4.1.4 | ✅ (added by doc-polish) |
| RGS | Yes — "Rescue Group Supervisor (RGS)" | §4.2.5 | ✅ |
| PPE | Yes — "personal protective equipment (PPE)" | §7.4 | ✅ (added by doc-polish) |
| TECC | Yes — "Tactical Emergency Casualty Care (TECC)" | §9.2 | ✅ |
| CCP | Yes — "Casualty Collection Point (CCP)" | §12 (Key Definitions) | ✅ |
| THREAT Protocol | N/A — not used in body | §12 only | ✅ |
| FIRESCOPE | Used in §1.5 as proper noun; not formally expanded | §1.5 / §13 | ⚠️ Flagged for PM (see cross-reference note) |

### Items Flagged for PM Audit

1. **M.A.R. vs MARCH (Language #2 / Prof #3).** Inspection flagged "M.A.R." in §4.2.4 as inconsistent with the document's only formal definition (MARCH, in §4.2). The doc-prose step presented this in FLAG-AND-CONFIRM mode and the user **rejected** the proposed change, retaining "M.A.R." as intentional tactical emphasis on the first three priorities of MARCH (Massive hemorrhage, Airway, Respiration). This conflicts with the doc-polish Work Order mandate, which called for resolving the inconsistency. **PM decision required:** accept the user's prose-step decision and leave M.A.R. as-is, or override and unify on MARCH throughout.

2. **§1.5 cross-reference: "FIRESCOPE 701" vs. References "FIRESCOPE California, ICS-701".** The body refers to the parent doctrine as "FIRESCOPE 701"; the References cite "FIRESCOPE California, ICS-701: Unified Response to Violence Incidents (2023)". These appear to denote the same standard but are not phrased identically. **PM/SME verification recommended** before final publication. Suggested unification: "FIRESCOPE California's ICS-701 doctrine" in §1.5.
